<div class="col-md-3 pr-0">
  ...
  @foreach($datasurat as $folders)
      <li class="nav-item" role="presentation">
        <a href="#tab{{ $folders->id }}" class="{{ $folders->id == 1 ? 'nav-link active' : 'nav-link' }}" id="{{$folders->id}}-tab" data-toggle="tab" role="tab" aria-controls="{{$folders->id}}" aria-selected="false" style="border-radius:0px">{{$folders->jenis}}</a>
      </li>
  @endforeach
  </ul>
</div>